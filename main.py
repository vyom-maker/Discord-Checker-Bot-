...
class CheckerSession:
    ...
    async def send_final_summary(self, title="🏁 Checker Completed"):
        ...
        # Send result files as DM to the user
        ...
        try:
            ...
            if result_files:
                await dm.send(content=f"📎 **Your Results** ({len(result_files)} file(s)):",
                              files=[discord.File(fp) for fp in result_files])
                await self.ctx.send("📬 Results have been sent to your DMs!")
            else:
                await self.ctx.send("📭 No result files found (0 hits/captures).")
        except discord.Forbidden:
            # User has DMs disabled, fall back to channel
            await self.ctx.send("⚠️ Couldn't DM you (DMs might be disabled). Sending here instead.")
            if result_files:
                await self.ctx.send(
                    content=f"📎 **Results** ({len(result_files)} file(s)):",
                    files=[discord.File(fp) for fp in result_files]
                )
